...
toList' [] _ = []
toList' l@((n, x) : xs) k | n == k    = x : toList' xs (k + 1)
...
evalP (Poly xs) pt = shift 0 xs
  where
    shift _ [] = zero
    shift k (y@(n, a) : ys) | k == n    = a + shift (k + 1) ys * pt
                            | otherwise = shift (k + 1) (y : ys) * pt

-- | Multiply two polys.
mul :: Ring a => Poly a -> Poly a -> Poly a
mul (Poly xs) (Poly ys) =
    sortSimplifyP $ Poly [ (n + m, a * b) | (n, a) <- xs, (m, b) <- ys ]
...
merge' _ p  [] = p
merge' _ [] q  = q
merge' f (x@(n, a) : xs) (y@(m, b) : ys) | n == m = (n, f a b) : merge' f xs ys
