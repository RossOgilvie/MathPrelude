...
module MathPrelude.SpecialFunctions.SpecialFunc
  (
...
import           MathPrelude.SpecialFunctions.Combinatorics
import           MathPrelude.Calculus.Convergence
import           MathPrelude.SpecialFunctions.Primes
...
gammaF z'
  ...
  where
    ...
    part4 = head ck + sum (zipWith (\c k → c/(z+fromReal k)) (tail ck) [1..])
...
zetaFHasse s = factor * converge (partialSums outer_sum)
  where
    factor = 1/(1-2**(1-s))
...
zetaFEuler s
  ...
  where
    euler p = 1/(1-p**(-s))
    terms = map (euler . fromInteger) primes
...
eulerConst ∷ Double
eulerConst = 0.57721566490153286
...
li x = eulerConst + log (log x) + sqrt x * res
  where
...
erfSeries z = 2 / sqrt pi * converge (partialSums terms)
  where
    terms = map term [0..]
    term n = sign n * z^(2*n+1)/factorial n / fromIntegral (2*n+1)

erfProd z = 2 / sqrt pi * converge (partialSums terms)
  where
