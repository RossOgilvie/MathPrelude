...
module MathPrelude.Constructions.Vector
  ...
  , crossV
  -- , normV, unitizeV
  , pattern V1, pattern V2, pattern V3, pattern V4, pattern V5
  ) where
...
takeAndCountV n xs = let n' = fromIntegral n; xs' = take n' xs in (length xs' == n', xs')
...
pattern V1 t <- (toTuple1 -> t)
    where V1 t = fromTuple1 t
pattern V2 t <- (toTuple2 -> t)
    where V2 t = fromTuple2 t
pattern V3 t <- (toTuple3 -> t)
    where V3 t = fromTuple3 t
pattern V4 t <- (toTuple4 -> t)
    where V4 t = fromTuple4 t
pattern V5 t <- (toTuple5 -> t)
    where V5 t = fromTuple5 t
